...
> **Superseded by Experiment 11.** The 1.8 SE / "below the bar" result above was measured against a small, provisional sample (n=120, single width). The full backtest across all widths at 3% distance (n=126, `EXPERIMENT.md` Experiment 11) clears the bar comfortably: 3.28 SE at $1 width, 2.97 SE at $2, 2.28 SE at $5. That is the standing result -- this section is kept as-is for the reasoning it demonstrates (why eyeballing a win rate is not enough), not as the current number. See `EXPERIMENT.md` and Headline Finding 1 in the bug-audit plan for the trading-cost caveat that applies to those cushions.
