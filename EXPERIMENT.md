...
*(Numbered 28, not 21 — 21 was already taken by the volatility track's own "Randomization null for the week-skip filter" above, which keeps its number since that track cross-references it throughout. Full standalone writeup, same content: `EXPERIMENT_28_VRP.md`.)*

**Hypothesis.** The existing edge, reported since Experiment 12/W4, comes from a VIX curve (VIX9D/VIX3M contango) threshold: skip a week when the curve flattens or inverts, because that is when the market expects more turbulence soon than later. That threshold was fitted — its 33rd-percentile cutoff was calibrated on the calmest stretch on record (2016-2017), a real, disclosed weakness (see README's "two caveats" section). A more direct way to state "the market is overpaying for this spread" is to price the identical spread twice — once with the implied-vol input this system already uses (VIX9D, skew-adjusted), once with trailing realized SPY volatility — and take the difference. Call it `vrp_edge`: zero by construction when the two volatilities agree, nonzero only when they diverge, which is what a volatility risk premium actually is.

**Method.** `pipeline/backtest/reconstruct.py`'s `replay()` already computes `credit` from `spread_value(spot, short_k, long_k, tau, sigma)` with `sigma` from VIX9D through the module's validated `k(vol)` skew adjustment. Adding the second side meant computing `value_realized = spread_value(...)` with `sigma` from a 20-day trailing annualized realized-vol series on SPY closes instead, reimplemented locally rather than importing `pipeline/options/vol.py` (that module constructs a live Alpaca client at import time and would have made this module require an API key just to import). `vrp_edge = credit - value_realized`. Both filters were then compared on identical footing: the same walk-forward rule (`walk_forward_threshold` in `vrp_measure.py`, mirroring `reconstruct.build_equity_curve`'s own contango mask), the same 60-week warmup and 33rd-percentile cutoff the live contango filter uses. No lookahead: the realized-vol series is trailing by construction, pinned by `test_realized_vol_series_no_lookahead`. Sign convention (`vrp_edge > 0` means the market is paying more for this spread than recent realized volatility justifies) pinned by `test_vrp_edge_sign_convention`.

**Result** (measured 2026-09-01, same 538-week 2016-2026 replay `reconstruct.py` already validates; 534 of 538 weeks have a full realized-vol lookback). `corr(vrp_edge, contango) = -0.13` — weak, independent information, not a restatement of the term-structure filter in dollar units. It also passes its own false-trip test: replayed against real (Feb 2024-2026) evidence-gate survivor cells, `vrp_edge` blocks 23.6% of real winning weeks (29/123), comfortably under the 30% bar. By that specific test, it is not mis-set. So the natural next question — does combining it with contango do better? — was tested directly:
...
`vrp_edge`-only and the AND-combination both beat the live filter on raw total P&L, by a wide margin for AND. **That is not the finding it looks like.** Both get there by giving up almost all of 2018's protection — the live filter turns 2018 from a -9.89 loss into a +0.26 near-wash; both `vrp_edge`-based rules leave it at -5.48 — and both run a 63-70% deeper max drawdown than the live filter. The extra total P&L comes from trading through calmer years the live filter currently sits out, not from handling 2018 better. OR, the conservative combination, matches the live filter's exact 2018/drawdown numbers but at a worse total P&L, i.e. it adds nothing contango doesn't already do alone. 2020 (COVID) shows the same pattern for `vrp_edge`-only (-0.08, barely better than doing nothing), though AND happens to recover it (+1.98) — inconsistent behavior across the two real crash-from-calm years in the sample is itself a reason for caution, not reassurance.

**Interpretation and decision.** `vrp_edge` is a real, honestly-computed measurement — the null-case property holds exactly, the sign convention is correct, it is independent of contango, and it is not mis-set by the project's own false-trip standard. All of that is true, and none of it is enough. **Decision: `vrp_edge` stays a reported measurement, not a gate, not a replacement, not an addition.** Three reasons: (1) optimizing for total P&L here reproduces the exact failure shape this project already caught itself making once (`reconstruct.py`'s own module docstring describes an earlier reconstruction that scored a respectable aggregate correlation while being wrong in opposite directions by volatility regime, a confident false finding that had to be retracted) — picking a filter for a higher aggregate total P&L while it quietly gives up protection on the two real crash-from-calm years in the sample is the same mistake in a different part of the codebase; (2) the whole point of the live filter, per README, is protecting against the tail, not maximizing return — a filter change that trades tail protection for a better average return is optimizing the opposite of what this system is for, even when the average genuinely improves; (3) every number this project's headline figures depend on remains bit-identical before and after this experiment (re-verified against the tracked `output/data/reconstruction_2016_2026.csv`). **Not wired into the live agent** — `select_spread()` is a pure function on plain dicts by design (no network calls inside the Picker), and a live `vrp_edge` would need a live realized-vol fetch threaded through `run_agent.py`; given the result above (a measurement that isn't gating anything), that wiring wasn't done.

**What would change this decision:** not a better total-P&L number. Evidence that a combination protects the tail *at least as well* as contango alone, tested against a genuinely held-out crash-from-calm period this filter wasn't built or compared against — 2018 and 2020 have now both been looked at twice (once to build contango, once here), so neither can serve as unbiased out-of-sample evidence for a third filter. Absent a new tail event or a pre-registered holdout, this line of investigation is exhausted for now.

**Scripts:** `pipeline/backtest/vrp_measure.py` (`walk_forward_threshold`, `compare_filters`, `compare_equity_curves`, `false_trip_rate_vrp_edge`), `tests/test_reconstruct_vrp.py`. Full writeup with the year-by-year P&L breakdown: `EXPERIMENT_28_VRP.md`.
