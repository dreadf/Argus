...
AUC improvement across Experiments 7 to 9 was noise throughout. Experiment 11
then reproduced the options-spread backtest as real code (`EXPERIMENT.md`):
three (distance, width) cells at 3% distance clear the 2-SE evidence-gate bar.

**Numbering here continues from 12.**
...
**Read that split honestly: our information set is limited** - daily bars, one asset, ~13 features. That is the regime where the literature says HAR wins. **Our prior should be that HAR-RV beats XGBoost here**, and Experiment 12 is designed to confirm or refute that, not to justify ML.
...
### Experiment 12 - Can we forecast SPY volatility at all?
...
### Experiment 13 - Does forecasting accuracy actually make money?

The experiment the research says most people skip.

- **What:** take Experiment 12's winner. Feed it into the spread backtest two ways - (a) as a skip filter (don't trade when high vol forecast), (b) as a distance scaler (sell further out when vol forecast is high)
...
### Experiment 14 - Predict the premium directly (the real target)
...
### Experiment 15 - Feature ablation: does `news_count` finally earn its place?
...
### Experiment 16 - Does the 40-stock panel add anything?
...
- **⛔ Gate:** must improve on Experiment 12's winner out of sample. If not, `panel.py` is retired honestly rather than kept for sentiment.

## Ordering, and what to test after this hackathon

| Priority | Experiment | Why this order |
|---|---|---|
| 1 | **12** | Everything downstream needs a volatility forecast that's been checked against a dumb baseline |
| 2 | **13** | Cheap, and it can kill the whole ML track early if forecasting doesn't convert to profit. Better to learn that before building more. |
| 3 | **14** | The real money question, but needs the reconstructed implied series from Exp 13/12 infrastructure |
| 4 | **15** | Self-contained, data already on disk, research-predicted, nice story |
| 5 | **16** | Most speculative, reuses the most existing code |

**Within the hackathon window, only Experiment 12 is realistically in scope** (Wednesday, timeboxed to 2h). **13 to 16 are the post-hackathon research track** - which is the honest way to end the write-up: here's what we tested, here's what we'd test next, here's why in that order.
