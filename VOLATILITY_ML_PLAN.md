...
active research plan: the 5-day direction question is closed with a documented
negative answer in `EXPERIMENT.md` (Experiments 0 to 9).
Experiment numbering continues from 10.

## The machine-learning direction
...
**Read that split honestly: our information set is limited** - daily bars, one asset, ~13 features. That is the regime where the literature says HAR wins. **Our prior should be that HAR-RV beats XGBoost here**, and Experiment 10 is designed to confirm or refute that, not to justify ML.
...
### Experiment 10 - Can we forecast SPY volatility at all?
...
### Experiment 11 - Does forecasting accuracy actually make money?

The experiment the research says most people skip.

- **What:** take Experiment 10's winner. Feed it into the spread backtest two ways - (a) as a skip filter (don't trade when high vol forecast), (b) as a distance scaler (sell further out when vol forecast is high)
...
### Experiment 12 - Predict the premium directly (the real target)
...
### Experiment 13 - Feature ablation: does `news_count` finally earn its place?
...
### Experiment 14 - Does the 40-stock panel add anything?
...
- **⛔ Gate:** must improve on Experiment 10's winner out of sample. If not, `panel.py` is retired honestly rather than kept for sentiment.
...
| 3 | **12** | The real money question, but needs the reconstructed implied series from Exp 10/11 infrastructure |
...
**Within the hackathon window, only Experiment 10 is realistically in scope** (Wednesday, timeboxed to 2h). **12–14 are the post-hackathon research track** - which is the honest way to end the write-up: here's what we tested, here's what we'd test next, here's why in that order.
